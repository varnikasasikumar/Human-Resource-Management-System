...
class SecurityUnitTest {
    ...
    @Test
    void testTokenProvider_validateAndClaims() {
        ...
        assertEquals("EMP001", jwtTokenProvider.getLoginIdFromToken(token));
        assertEquals("EMPLOYEE", jwtTokenProvider.getRoleFromToken(token));
    }

    @Test
    void testTokenProvider_invalidToken() {
        assertFalse(jwtTokenProvider.validateToken("invalid.token.here"));
    }

    @Test
    void testFilter_validTokenSetsSecurityContext() throws Exception {
        SecretKey key = io.jsonwebtoken.security.Keys.hmacShaKeyFor(secret.getBytes());
        String token = io.jsonwebtoken.Jwts.builder()
                .subject("HR001")
                .claim("role", "HR")
    ...
    @Test
    void testFilter_missingOrInvalidToken_contextNull() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        MockFilterChain filterChain = new MockFilterChain();
